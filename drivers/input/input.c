...
DECLARE_STATE_FUNC(idle)
{
	...
	pr_debug("[Input Booster] ******      State0 : Idle  index : %d, hmp : %d  cpu : %d\n", _this->index, _this->param[_this->index].hmp_boost, _this->param[_this->index].cpu_freq);
	...
	if(input_booster_event == BOOSTER_ON) {
		...
		//SET_BOOSTER;
		schedule_work(&_this->input_booster_set_booster_work);
...
DECLARE_STATE_FUNC(press)
{
	...
	if(input_booster_event == BOOSTER_OFF) {
		pr_debug("[Input Booster] ****        State : Press  index : %d, time : %d\n", _this->index, _this->param[_this->index].time);
		if(_this->multi_events == 0) {
			if(delayed_work_pending(&_this->input_booster_timeout_work)) {
				...
				pr_debug("[Input Booster] ****             schedule_delayed_work again  time : %d\n", _this->param[(_this->index > 0) ? _this->index-1 : _this->index].time);
				if(!delayed_work_pending(&_this->input_booster_timeout_work)) {
					pr_debug("[Input Booster] ****             schedule_delayed_work Re-again time : %d\n", _this->param[(_this->index > 0) ? _this->index-1 : _this->index].time);
					schedule_delayed_work(&_this->input_booster_timeout_work, msecs_to_jiffies(_this->param[(_this->index > 0) ? _this->index-1 : _this->index].time));
				}
		...
		if(delayed_work_pending(&_this->input_booster_timeout_work)) {
			pr_debug("[Input Booster] ******           cancel the pending workqueue for multi events\n");
			cancel_delayed_work(&_this->input_booster_timeout_work);
			schedule_delayed_work(&_this->input_booster_timeout_work, msecs_to_jiffies(_this->param[(_this->index > 0) ? _this->index-1 : _this->index].time));
		} else {
			pr_debug("[Input Booster] ****        State : Press  index : %d, time : %d\n", _this->index, _this->param[_this->index].time);
...
{
	int i, j, DetectedCategory = false, iTouchID = -1;
	...
	for(i=0;i<input_count;i++) {
		if (DetectedCategory) {
			...
			switch (input_events[i].code) {
				case BTN_TOUCH : 
					if(iTouchID >= 0) { // ***************** Checking Touch Event's ID whethere it is same with previous ID.
						if(!input_events[i].value && input_events[iTouchID].value < 0) {  // If this event is 'Release' 
							for(i=0;i<MAX_MULTI_TOUCH_EVENTS;i++) {
								TouchIDs[i] = -1;
							}
							pr_debug("[Input Booster] TOUCH EVENT - RELEASE - ID: 0x%x\n", input_events[iTouchID].value);
						} else {
							int SameID = -1, Blank = -1;
							for(j = 0;j < MAX_MULTI_TOUCH_EVENTS;j++) {
								pr_debug("[Input Booster] TOUCH EVENT - TouchIDs[j] : %d\n", TouchIDs[j]);
								if(TouchIDs[j] == input_events[iTouchID].value) {
									SameID = j;
								} else if(TouchIDs[j] < 0 && Blank < 0) {
									Blank = j;
								}
							}
							if(SameID < 0 && Blank > 0 && input_events[iTouchID].value > 0 && input_events[i].value) {  // If this Event's ID has never been received and this event is 'Press'
								TouchIDs[Blank] = input_events[iTouchID].value;
								pr_debug("[Input Booster] TOUCH EVENT - PRESS - ID: 0x%x\n", input_events[iTouchID].value);
							}
						}
					}
		...
		} else if (input_events[i].type == EV_ABS && input_events[i].code == ABS_MT_TRACKING_ID) {
				iTouchID = i;
			...
			if (input_events[i+1].type == EV_KEY) {
				switch (input_events[i+1].code) {
					...
					case BTN_MIDDLE : 
						...
						mouse_booster.input_booster_state(&mouse_booster, (input_events[i+1].value) ? BOOSTER_ON : BOOSTER_OFF );
						DetectedCategory = true;
						break;
					default : // ***************** Checking Keyboard Event
						pr_debug("[Input Booster] KEYBOARD EVENT - %s\n", (input_events[i+1].value) ? "PRESS" : "RELEASE");
						keyboard_booster.input_booster_state(&keyboard_booster, (input_events[i+1].value) ? BOOSTER_ON : BOOSTER_OFF );
			...
			if (input_events[0].type == EV_KEY && input_events[0].code == BTN_LEFT) {
				pr_debug("[Input Booster] MOUSE EVENT - %s\n", "WHELL");
				mouse_wheel_booster.input_booster_state(&mouse_wheel_booster, BOOSTER_ON);
				DetectedCategory = true;
			}
		}
	}
		 ...
		 unsigned int type, unsigned int code, int value)
{
	...
	if (is_event_supported(type, dev->evbit, EV_MAX)) {
		...
		if (type == EV_SYN && input_count > 0) {
			...
			pr_debug("[Input Booster 1] type = %x, code = %x, value =%x\n", type, code, value);
