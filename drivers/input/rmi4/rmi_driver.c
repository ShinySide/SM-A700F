 ...
 */
int rmi_device_reflash(struct rmi_device *rmi_dev)
{
 ...
 *
 */
int rmi_device_reset(struct rmi_device *rmi_dev)
{
	struct pdt_entry pdt_entry;
	int page;
	struct device *dev = &rmi_dev->dev;
...
static int rmi_driver_remove(struct rmi_device *rmi_dev)
{
	...
	disable_sensor(rmi_dev);
	if (data->rmi_input)
		rmi_input_dev_unregister(data->rmi_input);
...
static int create_unified_input_device(struct rmi_driver_data * data)
{
	...
	data->rmi_input = rmi_input_dev_alloc(&rmi_dev->dev, data, "%s/input0", NULL);
	if (!data->rmi_input) {
		return -ENOMEM;
	}
...
static int register_unified_input_device(struct rmi_driver_data * data, struct device * dev)
{
	...
	return rmi_input_dev_register(data->rmi_input);
}

static const char *GPIO_LABEL = "attn";
int rmi_driver_redetect(struct rmi_device *rmi_dev)
{
	int retval = 0;
	struct rmi_driver_data *data = dev_get_drvdata(&rmi_dev->dev);
	struct rmi_device_platform_data *pdata = to_rmi_platform_data(rmi_dev);

	if(! rmi_dev->xport->probe_succeeded) //don't run this function when during probe.
		return 0;
	retval = rmi_device_reset(rmi_dev);
	if (retval)
		dev_warn(&rmi_dev->dev, "RMI initial reset failed! Continuing in spite of this.\n");
	...
	if (retval < 0) {
		 ...
		 */
		dev_warn(&rmi_dev->dev, "Could not read PDT properties from %#06x. Assuming 0x00.\n",
			 PDT_PROPERTIES_LOCATION);
	}
	retval = rmi_count_irqs(rmi_dev);
	if (retval) {
		dev_err(&rmi_dev->dev, "IRQ counting for %s failed with code %d.\n",
			pdata->sensor_name, retval);
		goto err_free_data;
	}

	data->irq_status = devm_kzalloc(&rmi_dev->dev,
			BITS_TO_LONGS(data->irq_count)*sizeof(unsigned long),
			GFP_KERNEL);
	if (!data->irq_status) {
		dev_err(&rmi_dev->dev, "Failed to allocate irq_status.\n");
		retval = -ENOMEM;
		goto err_free_data;
	}

	data->current_irq_mask = devm_kzalloc(&rmi_dev->dev, data->num_of_irq_regs,
				GFP_KERNEL);
	if (!data->current_irq_mask) {
		dev_err(&rmi_dev->dev, "Failed to allocate current_irq_mask.\n");
		retval = -ENOMEM;
		goto err_free_data;
	}

	data->irq_mask_store = devm_kzalloc(&rmi_dev->dev,
		BITS_TO_LONGS(data->irq_count)*sizeof(unsigned long),
		GFP_KERNEL);
	if (!data->irq_mask_store) {
		dev_err(&rmi_dev->dev, "Failed to allocate mask store.\n");
		retval = -ENOMEM;
		goto err_free_data;
	}
	...
	if (retval) {
		dev_err(&rmi_dev->dev, "PDT scan for %s failed with code %d.\n",
			pdata->sensor_name, retval);
		goto err_free_data;
	}

	if (!data->f01_dev) {
		dev_err(&rmi_dev->dev, "missing F01 device!\n");
		retval = -EINVAL;
		goto err_free_data;
	}
...
err_input_free_device:
	if (data->rmi_input) {
		rmi_input_dev_free(data->rmi_input);
		data->rmi_input = NULL;
	}
err_free_data:
 	if (data->rmi_input)
