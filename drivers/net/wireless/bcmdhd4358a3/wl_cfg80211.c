 ...
 *
 * $Id: wl_cfg80211.c 551178 2015-04-22 14:32:52Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static void wl_flush_eq(struct bcm_cfg80211 *cfg)
{
	...
	while (!list_empty(&cfg->eq_list)) {
		...
		sec_debug_aux_log(SEC_DEBUG_AUXLOG_WIFI, "D:eq_list-0x%p", &e->eq_list);
		list_del(&e->eq_list);
		sec_debug_aux_log(SEC_DEBUG_AUXLOG_WIFI, "F:eq_list-0x%p", &e->eq_list);
		kfree(e);
	}
	...
	const wl_event_msg_t *msg, void *data)
{
	...
	sec_debug_aux_log(SEC_DEBUG_AUXLOG_WIFI, "M:eq_list-0x%p", &e->eq_list);
	if (unlikely(!e)) {
	...
	sec_debug_aux_log(SEC_DEBUG_AUXLOG_WIFI, "A:eq_list-0x%p", &e->eq_list);
	wl_unlock_eq(cfg, flags);
