...
#define ST_GYRO_SKIP_DELAY			300000000L /* 300ms */
...
static irqreturn_t st_lsm6ds3_accel_gyro_trigger_handler(int irq, void *p)
{
	int len;
	static int pr_cnt;
	static s64 pr_timestamp, skip_time_prev;
	s64 pr_timestamp_current = 0, skip_time_current = 0;
	...
	if(sdata->sindex == ST_INDIO_DEV_ACCEL) {
		...
		if (sdata->cdata->skip_gyro_data == true) {
			...
			if (((s64)ST_GYRO_SKIP_DELAY) <= (skip_delay * (s64)sdata->cdata->skip_gyro_cnt) ||
