...
#define ST_LSM6DS3_SA_DYNAMIC_THRESHOLD			150	// mg
			...
			u8 reg_addr, u8 mask, u8 data)
{
	...
	dev_info(common_data->dev, "%s addr= %x, old= %x, new= %x\n",
			__func__, reg_addr, bak_data, new_data);
	...
	case ST_INDIO_DEV_GYRO:
		if (test & 1) {
			...
			if (err < 0) {
				...
				pr_info("%s, fail cal = %d,%d,%d\n", __func__, sdata->cdata->gyro_cal_data[0], sdata->cdata->gyro_cal_data[1], sdata->cdata->gyro_cal_data[2]);
				goto restore_regs;
			}
			else{
				pr_info("%s, pass cal = %d,%d,%d\n", __func__, sdata->cdata->gyro_cal_data[0], sdata->cdata->gyro_cal_data[1], sdata->cdata->gyro_cal_data[2]);
			}
		}
		if (test & 2) {
