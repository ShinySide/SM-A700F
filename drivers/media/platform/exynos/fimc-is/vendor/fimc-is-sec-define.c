...
int fimc_is_sec_readcal_eeprom(struct device *dev, int id)
{
	...
	fimc_is_i2c_write(client, 0xA00, 0x00);
#else
	ret = fimc_is_i2c_read(client, cal_map_version, EEP_HEADER_VERSION_START_ADDR_FRONT, FIMC_IS_CAL_MAP_VER_SIZE);
	if (unlikely(ret)) {
		err("failed to fimc_is_i2c_read (%d)\n", ret);
		ret = -EINVAL;
		fimc_is_i2c_config(client, false);
	...
	if(cal_map_version[0] != 'V') {
		printk(KERN_INFO "Camera: Front camera OTP is not available\n");
...
crc_retry:
	...
	if (IS_ERR(key_fp)) {
		...
		if (IS_ERR(dump_fp)) {
			...
			if (write_data_to_file("/data/media/0/dump/eeprom_cal.bin", cal_buf, FIMC_IS_DUMP_CAL_SIZE, &pos) < 0) {
